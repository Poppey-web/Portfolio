      ...
      <section id="projects" className="py-20 sm:py-32 px-4 sm:px-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8">
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 md:pb-0 w-full md:w-auto">
            {categories?.map(cat => (
              <button key={cat} onClick={() => setFilter(cat)} className={`px-5 py-2 text-[9px] font-bold uppercase tracking-widest rounded-full border transition-all whitespace-nowrap ${filter === cat ? 'bg-white text-black border-white' : 'border-zinc-800 text-zinc-400 hover:border-zinc-600'}`}>
                {cat}
              </button>
            ))}
          </div>
      ...
      <section id="expertise" className="py-20 sm:py-32 px-4 sm:px-6 bg-zinc-950 border-y border-zinc-900">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
            <div>
              <h2 className="text-4xl sm:text-6xl font-bold tracking-tighter mb-8 text-white uppercase">{profile?.expertise?.title}</h2>
              <div className="text-zinc-400 text-base sm:text-lg leading-relaxed mb-12">{profile?.expertise?.description}</div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                {profile?.expertise?.skills?.map((skill, idx) => (
                  <div key={idx} className="flex items-center gap-4 text-white group">
                    <div className="p-2 bg-zinc-900 border border-zinc-800 rounded-sm group-hover:border-zinc-600 transition-colors"><Zap size={16} className="text-zinc-400" /></div>
                    <span className="font-bold uppercase tracking-widest text-[10px] text-zinc-300">{skill.label}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section id="about" className="py-20 sm:py-32 px-4 sm:px-6 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          <div className="lg:col-span-7">
            <h2 className="text-4xl sm:text-6xl font-bold tracking-tighter mb-8 text-white uppercase">{profile?.bioTitle}</h2>
